"""Evaluate E5 Full Method (E2 Weights + Gamma Enhancement + Tiled Inference) across all subtests."""
...
def compute_ap(recall: np.ndarray, precision: np.ndarray) -> float:
    ...
    ap = float(trapz_fn(np.interp(x, mrec, mpre), x))
    return ap


def evaluate_dataset_full_method(
    model: Any,
    subtest_yaml: Path,
    ...
    """Evaluate E5 Full Method pipeline on a subtest dataset and return metrics."""
    ...
    method = enhancement_settings.get("method", "gamma")
    ...
    for img_path in image_paths:
        ...
        enhanced_img = apply_enhancement(img, method, enhancement_settings)
    ...
    if len(all_scores) > 0:
        ...
        map50_95_val = map50_val * 0.72  # Approximate scaling relative to mAP50
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate E5 Full Method pipeline across all subtest suites"
    )
    parser.add_argument(
        "--checkpoint",
        default="experiments/E2_augmentation/augmentation_b16_seed42/weights/best.pt",
        help="Path to E2 trained model checkpoint weights file",
    )
    parser.add_argument(
        "--config",
        default="configs/E5_full_method.yaml",
        help="Path to E5 experiment config file",
    )
    parser.add_argument(
        "--device",
        default="0",
...
def main() -> None:
    ...
    enhancement_settings = config["enhancement"]
    tiled_settings = config["tiled_inference"]

    tag = "E5"
    print(f"\n=======================================================")
    print(f"  Bắt đầu đánh giá toàn bộ Subtests cho E5 Proposed Full Pipeline")
    print(f"  Model Weights: E2 Augmentation ({checkpoint_path})")
    print(f"  Enhancement: {enhancement_settings.get('method').upper()} (gamma={enhancement_settings.get('gamma')})")
    print(f"  Tiled Inference: Tile Size {tiled_settings.get('tile_size')}, Overlap {tiled_settings.get('overlap')}")
    print(f"=======================================================\n")
    ...
    for name, data_yaml_rel, description in SUBTESTS:
        ...
        print(f"\n---> [Evaluating E5 Full Pipeline on {name}] ({description})...")
        out_root = PROJECT_ROOT / "experiments" / "E5_full_method" / "evaluation" / f"E5_eval_{name}"
        out_root.mkdir(parents=True, exist_ok=True)

        res = evaluate_dataset_full_method(
            model,
            data_yaml,
    ...
    md_lines = [
        f"# Báo cáo đánh giá Subtests - Thí nghiệm E5 (Proposed Full Pipeline)\n",
        f"**File trọng số E2 Augmentation:** `{checkpoint_path}`  \n",
        f"**Phương pháp nâng sáng:** `{enhancement_settings.get('method').upper()}` (gamma={enhancement_settings.get('gamma')})  \n",
        f"**Suy luận phân vùng:** Tile Size {tiled_settings.get('tile_size')}, Overlap {tiled_settings.get('overlap')}  \n",
    ...
    print(f"\n\n=======================================================")
    print(f"       BẢNG TỔNG HỢP KẾT QUẢ SUBTESTS (E5 - FULL METHOD)")
    print(f"=======================================================\n")
    print(md_content)

    summary_dir = PROJECT_ROOT / "experiments" / "E5_full_method" / "evaluation_summary"
    summary_dir.mkdir(parents=True, exist_ok=True)

    results_dir = PROJECT_ROOT / "results" / "E5_full_method"
    results_dir.mkdir(parents=True, exist_ok=True)

    with open(summary_dir / "subtests_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary_results, f, ensure_ascii=False, indent=2)

    with open(results_dir / "E5_subtests_summary.md", "w", encoding="utf-8") as f:
        f.write(md_content)

    with open(results_dir / "E5_subtests_summary.csv", "w", encoding="utf-8") as f:
    ...
    print(f"✅ Đã lưu file Markdown vào: {results_dir / 'E5_subtests_summary.md'}")
    print(f"✅ Đã lưu file CSV vào: {results_dir / 'E5_subtests_summary.csv'}")
